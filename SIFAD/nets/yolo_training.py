...
class ACCRLoss(nn.Module):
    def __init__(self):
        super(ACCRLoss, self).__init__()
        self.scale_loss = nn.CrossEntropyLoss()

    def __call__(self, scale_logits, labels, rcho):
        # -------------------------------------------#
        #   ACCRLoss 用于尺度不变特征和尺度特征去相关
        # -------------------------------------------#
        # predictions = torch.max(scale_logits, dim=1)[1].squeeze()
        # print("logits:", scale_logits)
        # print("label:", labels)
        labels = labels.long()
        scale_loss = self.scale_loss(scale_logits, labels)
        total_loss = scale_loss + 10 * rcho
        return total_loss


class ACCRLoss_regression(nn.Module):
    def __init__(self):
        super(ACCRLoss_regression, self).__init__()
        self.scale_loss = nn.MSELoss(reduction="mean")
...
class YOLOLoss(nn.Module):
    ...
    def comupte_distill_loss(self, predictions, t_predictions, T=20):
        # -------------------------------------------#
        #   对输入进来的预测结果进行reshape
        #   bs, 255, 20, 20 => bs, 3, 20, 20, 85
        #   bs, 255, 40, 40 => bs, 3, 40, 40, 85
        ...
        for i, pi in enumerate(predictions):
            ...
            # bbox
            pi_1 = pi[..., :4].sigmoid()
            t_pi_1 = t_pi[..., :4].sigmoid()

            # pi_1 = pi[..., :4]
            # t_pi_1 = t_pi[..., :4]

            box_loss += DboxLoss(pi[..., :4].sigmoid(), t_pi[..., :4].sigmoid())
            # class
            cls_loss += DclsLoss(F.log_softmax(pi[..., 5:] / T, dim=-1),
                                 F.softmax(t_pi[..., 5:] / T, dim=-1)) * (T * T)
            # obj
            obj_loss += DobjLoss(pi[..., 4], t_pi[..., 4])
